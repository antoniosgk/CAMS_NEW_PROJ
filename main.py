...
from station_level_lookup import build_and_save_station_level_lookup # type: ignore
...
# analysis station selection mode:
...
#   "single" -> use LOOKUP_STATION_IDX
#   "list"   -> use LOOKUP_STATION_IDXS
#   "all"    -> all valid stations
LOOKUP_STATION_SELECTION = "all"

LOOKUP_STATION_IDX = 5
LOOKUP_STATION_IDXS = [1, 5, 8]
...
if LOOKUP_STATION_SELECTION == "single":
    ...
    LEVEL_LOOKUP_PATH = f"{lookup_dir}/station_level_timeseries_idx{LOOKUP_STATION_IDX}.parquet"

elif LOOKUP_STATION_SELECTION == "list":
    tag = "_".join(str(x) for x in LOOKUP_STATION_IDXS)
    HORIZONTAL_LOOKUP_PATH = f"{lookup_dir}/station_horizontal_lookup_list_{tag}.parquet"
    LEVEL_LOOKUP_PATH = f"{lookup_dir}/station_level_timeseries_list_{tag}.parquet"

elif LOOKUP_STATION_SELECTION == "all":
    ...
    raise ValueError("LOOKUP_STATION_SELECTION must be 'single', 'list', or 'all'")
...
def main():
    ...
    if BUILD_K_LOOKUP:
        ...
        end_dt=END_DT,
        step_minutes=30,
        station_selection=LOOKUP_STATION_SELECTION,
        station_idx=LOOKUP_STATION_IDX,
        station_idxs=LOOKUP_STATION_IDXS,
